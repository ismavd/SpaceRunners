...
public class Level 
{
	...
	private void init(String filename, boolean checkpointReached) 
	{
		...
		// Escaneamos los pixeles de la imagen de arriba-izquierda a abajo-derecha.
		int lastPixel = -1;
		for (int pixelY = 0; pixelY < pixmap.getHeight(); pixelY++) 
		{
			for (int pixelX = 0; pixelX < pixmap.getWidth(); pixelX++) 
			{
				...
				// Comparamos el color de cada pixel de la imagen y si corresponde con alguno de los
				// asignados a los objetos del nivel, creamos un objeto de ese tipo en esa coordenada o pixel x e y.
				...
				else if (BLOCK_TYPE.PLATFORM.sameColor(currentPixel)) //Plataforma flotante
				{
					if (lastPixel != currentPixel) 
					{
						obj = new Platform();
				...
				else if (BLOCK_TYPE.MOVING_PLATFORM.sameColor(currentPixel)) //Plataforma flotante
				{
					if (lastPixel != currentPixel) 
					{
					...
					{
						movingPlatforms.get(movingPlatforms.size - 1).increaseLength(1);
					}
				}
				else if (!checkpointReached && BLOCK_TYPE.PLAYER_SPAWNPOINT.sameColor(currentPixel)) // Punto aparición del jugador
				{
					obj = new BunnyHead();
					offsetHeight = -3.0f;
					obj.position.set(pixelX, baseHeight * obj.dimension.y + offsetHeight); 
				...
				else if (BLOCK_TYPE.ITEM_FEATHER.sameColor(currentPixel)) // Pluma (Barra energética)
				{
					obj = new Feather();
					offsetHeight = -1.5f;
					obj.position.set(pixelX, baseHeight * obj.dimension.y + offsetHeight);
				...
				else if (BLOCK_TYPE.ITEM_GOLD_COIN.sameColor(currentPixel)) // Moneda de oro (Tuerca)
				{
					obj = new GoldCoin();
					offsetHeight = -1.5f;
					obj.position.set(pixelX, baseHeight * obj.dimension.y + offsetHeight);
				...
				else if (BLOCK_TYPE.BOX.sameColor(currentPixel)) //Caja metálica
				{
					obj = new Box();
					offsetHeight = -1.5f;
					obj.position.set(pixelX, baseHeight * obj.dimension.y + offsetHeight);
				...
				else if (BLOCK_TYPE.GOAL.sameColor(currentPixel)) //Punto de meta 
				{
					obj = new Goal();
					offsetHeight = -7.0f;
				...
				else  // Color de pixel u objeto desconocido
				{
					...
					Gdx.app.error(TAG, "Unknown object at x<" + 
									   pixelX + "> y<" + pixelY + ">: r<" + r + "> g<" + g + "> b<" + b + "> a<" + a + ">");
				}
		...
		// Creamos y colocamos los objetos decorativos del nivel (Nubes y montañas)
